...
class QueryRepository:
    ...
    async def create_query_entry(self, entry: QueryRequest) -> QueryEntry:
        entry = QueryEntry(uuid=str(uuid.uuid4()), type=entry.query_type, progress=QueryProgress.QUEUED, topics=entry.topics,
                           start_year=entry.start_year, end_year=entry.end_year, distance=entry.distance, min_citations=entry.min_citations, results=None)
        insert_query = "INSERT INTO queries (uuid, type, progress, topics, start_year, end_year, distance, min_citations, results) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9);"
    ...
    async def get_query_summary(self, uuid: str) -> QueryEntry:
        select_query = "SELECT uuid, type, progress, topics, start_year, end_year, distance, min_citations FROM queries WHERE uuid = $1;"
        row = await self.conn.fetchrow(select_query, uuid)
        if row == None:
            return None
        results_map = {**{i: row[i] for i in row.keys() if i != 'results'},
